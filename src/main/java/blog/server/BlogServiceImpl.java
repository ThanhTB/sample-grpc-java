...
public class BlogServiceImpl extends BlogServiceGrpc.BlogServiceImplBase {
    ...
    public BlogServiceImpl(MongoClient client) {
        MongoDatabase db = client.getDatabase("movie_db");
    ...
    public void readBlog(BlogId request, StreamObserver<Blog> responseObserver) {
        if (request.getId().isEmpty()) {
            responseObserver.onError(Status.INVALID_ARGUMENT.withDescription("The blog ID cannot be empty").asRuntimeException());
            return;
        }
        ...
        if (result == null) {
            responseObserver.onError(Status.NOT_FOUND.withDescription("Blog was not found").augmentDescription("BlogId: " + id).asRuntimeException());
            return;
        }
    ...
    public void deleteBlog(BlogId request, StreamObserver<Empty> responseObserver) {
        ...
        if (result.getDeletedCount() == 0) {
            responseObserver.onError(Status.NOT_FOUND.withDescription("Blog was not found").asRuntimeException());
            return;
        }
